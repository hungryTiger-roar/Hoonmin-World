...
@Composable
fun AdminAttractionManagementScreen(
        ...
        Column(modifier = Modifier.padding(16.dp).navigationBarsPadding()) {
            ...
            LazyColumn(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                items(attractions, key = { it.attId }) { attraction ->
                    val isAble = attraction.attAble
                    val cardColor = if (isAble) Color.White else Color(0xFFE0E0E0)
                    Card(modifier = Modifier.fillMaxWidth()) {
                        Column(
                            ...
                            Text(attraction.attName ?: "-", fontWeight = FontWeight.Bold)
                            Text("최대 탑승 인원: ${attraction.attCapacity}명")
                            Text("카테고리: ${attraction.attCategory ?: "-"}")
                            Row(
                                modifier = Modifier.fillMaxWidth(),
                                horizontalArrangement = Arrangement.End,
                                verticalAlignment = Alignment.CenterVertically
                            ) {
                                val toggleLabel = if (isAble) "운영중" else "운영중단"
                                val toggleColor = if (isAble) Color(0xFF5CB85C) else Color(0xFF9E9E9E)
                                SmallActionButton(label = toggleLabel, containerColor = toggleColor) {
